...
DISCORD_WEBHOOK_EQUIPE = "https://discord.com/api/webhooks/1452314030357348353/-ty01Mp6tabaM4U9eICtKHJiitsNUoEa9CFs04ivKmvg2FjEBRQ8CSC_PJtSD91ZkrvUi" # Webhook para notificações da equipe (ex: 10 min antes da pausa, AGORA TAMBÉM AGENDAMENTO)
DISCORD_WEBHOOK_GESTAO = "https://discord.com/api/webhooks/1452088104616722475/mIVeSKVD0mtLErmlTt5QqnQvYpDBEw7TpH7CdZB0A0H1Ms5iFWZqZdGmcRY78EpsJ_pI" # Webhook para notificações da gestão (ex: início/fim de pausa)
...
if supabase:
    ...
    if not st.session_state.logado:
        ...
        if u_info['tipo'] == 'Supervisor':
            ...
            hoje_iso = get_now().date().isoformat()
            pausa_resp = supabase.table('escalas').select('*').eq('email', st.session_state.user_atual).eq('data', hoje_iso).limit(1).execute()
