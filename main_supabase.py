            ...
            if menu == "Liberar Pausa":
                ...
                with tab_add:
                    with st.form("add_user"):
                        n_f = st.text_input("Nome*"); e_f = st.text_input("E-mail*").lower(); s_f = st.text_input("Senha*")
                        ...
                        if st.form_submit_button("💾 SALVAR"):
                            ...
                            st.rerun()
                with tab_del:
                    lista = [f"{u['nome']} ({u['email']})" for u in usuarios_resp.data if u['email'] != st.session_state.user_atual]
                    sel_del = st.selectbox("Remover:", lista)
                    cod_del = st.text_input("Código Mestre:", type="password")
                    if st.button("🗑️ DELETAR"):
                        if cod_del == CODIGO_MESTRE_GESTAO:
                            supabase.table('usuarios').delete().eq('email', sel_del.split('(')[-1].replace(')','')).execute()
                            st.rerun()
                ...
                if esc.data:
                    sel_un = st.selectbox("Selecione:", [f"{x['nome']} ({x['email']})" for x in esc.data])
                    ...
                    <script>
                        ...
                        var x = setInterval(function() {{
                            ...
                            if (diff <= 0) {{
                                ...
                                new (window.AudioContext || window.webkitAudioContext)().createOscillator().start();
