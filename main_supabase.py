...
# --- CONFIGURAÇÕES ---
# Lembrete: No GitHub, utilize st.secrets para estas variáveis por segurança
...
# --- UI E ESTILO (CORREÇÃO DO SELECTBOX) ---
...
<style>
    ...
    /* --- CORREÇÃO DO TEXTO INVISÍVEL NO SELECTBOX --- */
    /* Força o texto selecionado a aparecer em preto */
    div[data-baseweb="select"] > div {
        color: #262730 !important;
        -webkit-text-fill-color: #262730 !important;
    }
    
    /* Garante visibilidade no componente específico do Streamlit */
    .stSelectbox div[role="button"] > div {
        color: #262730 !important;
        opacity: 1 !important;
    }

    /* Estilização das opções dentro do menu suspenso */
    [data-baseweb="popover"] li {
        color: #262730 !important;
    }

    /* Layout Geral */
    .logo-qp { font-family: 'Arial Black', sans-serif; font-size: 35pt; color: #004a99; text-align: center; }
    ...
    .stButton > button[kind="primary"] { background: #004a99 !important; color: white !important; }
</style>
    ...
    if not st.session_state.logado:
        ...
        if st.button("ALTERAR SENHA"):
            if len(nova) >= 6 and nova == confirma:
                ...
                st.rerun()
            else: st.error("❌ Verifique a senha (mín. 6 caracteres).")
        ...
        if any(x in cargo for x in ['admin', 'supervisor', 'gestão']):
            ...
            if menu == "Liberar Pausa":
                ...
                if not at: st.warning("⚠️ Sem atendentes.")
                else:
                    ...
                    if st.button("✅ AUTORIZAR PAUSA"):
                        ...
                        enviar_discord(DISCORD_WEBHOOK_EQUIPE, f"🔔 {usuarios_db[alvo]['nome']}, pausa liberada!")
                        st.success("✅ Autorizado!")

            elif menu == "Gestão de Equipe":
                tab_add, tab_del = st.tabs(["➕ Adicionar", "🗑️ Remover"])
                with tab_add:
                    with st.form("add_user"):
                        ...
                        if st.form_submit_button("💾 SALVAR"):
                            ...
                            st.rerun()
                # (O restante do menu de remoção e histórico segue a mesma lógica anterior)
            ...
            # (Lógica do cronômetro original preservada aqui...)
